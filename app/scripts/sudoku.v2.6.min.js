var texts = {
  ...
  options = {
    ...
    load: function () {
      ...
      for (b = 0; b < board.arr.length; b++)
        for (c = 0; c < board.arr[b].length; c++) board.arr[b][c] !== board.arr_original[b][c] && a.push([b, c]);
      a = a.sort();
      board.numberOfMoves = utility.countEmtpyCells();
      board.load({
        arr: board.arr,
    ...
    Import: function (a) {
      if (a = a || prompt(texts.imp))
        if (81 != a.length) utility.alert.say(texts.import_invalid);
        else {
          var b, c = "";
          for (b = 0; 81 > b; b += 9) c += a.slice(b, b + 9), 70 > b && (c += ",");
          c = c.split(",");
          for (b = 9; b--;)
            for (c[b] = c[b].split(""), a = 9; a--;) c[b][a] = 0 == c[b][a] ? "" : c[b][a] | 0;
          b = c;
    ...
    transformArray: function (a) {
      ...
      for (d = 0; 3 > d; d++)
        for (e = 0; 3 > e; e++) {
          c = [];
          for (f = 0; 3 > f; f++)
            for (h = 0; 3 > h; h++) c.push(a[f + 3 * d][h + 3 * e]);
          b.push(c)
        }
      return b
    },
  ...
  board = {
    ...
    domFragments: {
      userCell: '<div><div><span></span></div><button title="' + texts.clear_cell + '">&times;</button></div>'
    },
    init: function (a) {
      this.container = a;
    ...
    checkWin: function (a, b) {
      ...
      for (var c = utility.flattenBoard(this.arr).join(""), d = 1; 9 >= d; d++)
        if (9 > c.split(d).length - 1) return utility.alert.say(texts.loose), !1;
      board.endGame()
    },
    ...
    makeTable: function (a) {
      ...
      for (c = 0; 3 > c; c++)
        for (d = $("<tr>").appendTo(b), e = 0; 3 > e; e++) f = $("<td>"), a + 1 && f.data("pos", a + "" + (3 * c + e)), d.append(f);
      return b
    },
    populate: function () {
      ...
      for (var c = {
        ...
        for (var f = 0; 9 > f; f++) {
          ...
          for (var h = utility.generateNumber(); !this.checkNumber.apply(c, [h, [e, f], !0]) && b[1];) {
            a++;
            if (5E4 < a) return; - 1 != d.indexOf(h) ? h = utility.generateNumber() : (d.push(h), h = utility.generateNumber(), b[1]--)
          }
          if (!b[1]) {
            b[0]--;
            c.arr[e].length = 0;
    ...
    checkNumber: function (a, b, c) {
      ...
      e = {
        ...
        row: function (b) {
          ...
          for (b = 0; 3 > b; b++)
            for (g = 0; 3 > g; g++)
              if (a == d.arr[3 * e + b][3 * subRow + g]) return !0;
          return !1
        },
        col: function (b) {
          if (3 > b[0] && c) return !1;
          ...
          for (g = 0; 3 > g; g++)
            for (k = 0; 3 > k; k++)
              if (a == d.arr[e + 3 * g][b + 3 * k]) return !0;
          return !1
        }
      };
    ...
    load: function (a) {
      a = a || {};
      var b = a.filledCells || [],
        c = this.generateMainTable(),
        d = c.find("table.box"),
        e = this,
        f, h, g, k, l, m, n = this.mainBoard.data("timeout") || null,
        p = 200;
      ...
      n = setTimeout(function () {
        ...
        for (len = e.arr.length; f < len; f++)
          for (g = d.eq(f), h = 0; h < len; h++) l = g.find("> tbody > tr > td").eq(h), k = board.arr[f][h], "" == k || 0 != b.length && f + "" + h == b[0].join("") ? (l.addClass("user").html(board.domFragments.userCell), "" != k && (l.addClass("filled").find("> div span:first").html(k), b.splice(0, 1))) : l.html("<span>" + board.arr[f][h] + "</span>").removeAttr("class");
